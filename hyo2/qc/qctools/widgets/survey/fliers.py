...
class FliersTab(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent_win, prj):
        ...
        self.check_export_proxies_ffv8 = None
        self.check_export_heights_ffv8 = None
        self.check_export_curvatures_ffv8 = None
        ...
        # - version switcher
        ffvs_hbox = QtWidgets.QHBoxLayout()
        self.vbox.addLayout(ffvs_hbox)
        ffvs_hbox.addStretch()
        self.flierFinderVersions = QtWidgets.QGroupBox("Version")
        ffvs_hbox.addWidget(self.flierFinderVersions)
        ffvs_hbox.addStretch()
        self.slider_ff_version = None
        self._ui_settings_ffvs()

        self.vbox.addStretch()

        self.flierFinderV8.hide()
    ...
    def keyPressEvent(self, event):
        ...
        if event.modifiers() == QtCore.Qt.ControlModifier:
            ...
            if key == QtCore.Qt.Key_7:
                self.view_version(version=7)

            if key == QtCore.Qt.Key_8:

                self.view_version(version=8)
    ...
    def _ui_settings_ffvs(self):
        hbox = QtWidgets.QHBoxLayout()
        # hbox.setContentsMargins(10, 5, 5, 10)
        self.flierFinderVersions.setLayout(hbox)
        self.flierFinderVersions.setMaximumWidth(180)

        hbox.addStretch()

        text_value = QtWidgets.QLabel("v7")
        text_value.setAlignment(QtCore.Qt.AlignRight)
        text_value.setFixedWidth(40)
        hbox.addWidget(text_value)

        self.slider_ff_version = QtWidgets.QSlider(QtCore.Qt.Horizontal)
        self.slider_ff_version.setRange(7, 8)
        self.slider_ff_version.setSingleStep(1)
        self.slider_ff_version.setValue(7)
        self.slider_ff_version.setTickInterval(1)
        self.slider_ff_version.setTickPosition(QtWidgets.QSlider.TicksBelow)
        self.slider_ff_version.setMinimumWidth(60)
        # noinspection PyUnresolvedReferences
        self.slider_ff_version.valueChanged.connect(self.on_changed_version_switcher)
        hbox.addWidget(self.slider_ff_version)

        text_value = QtWidgets.QLabel("v8 beta")
        text_value.setAlignment(QtCore.Qt.AlignLeft)
        text_value.setStyleSheet("QLabel { color: #D73232;}")
        text_value.setFixedWidth(40)
        hbox.addWidget(text_value)

        hbox.addStretch()

    def on_changed_version_switcher(self) -> None:
        self.view_version(version=self.slider_ff_version.value())

    def view_version(self, version: int) -> None:
        logger.debug("view version %d" % version)

        if version == 8:

            if self.flierFinderV8.isHidden():
                self.flierFinderV7.hide()
                self.flierFinderV8.show()
            else:
                self.flierFinderV7.show()
                self.flierFinderV8.hide()
            return

        if self.flierFinderV7.isHidden():
            self.flierFinderV7.show()
            self.flierFinderV8.hide()
        else:
            self.flierFinderV7.hide()
    ...
    def _ui_execute_ffv8(self):
        ...
        button.setText("Find fliers v8 beta")
