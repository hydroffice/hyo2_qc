...
class BAGQATab(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent_win, prj):
        ...
        # - bag qa v1
        self.bagqav1 = QtWidgets.QGroupBox("BAG QA v1")
        self.bagqav1.setStyleSheet("QGroupBox::title { color: rgb(155, 155, 155); }")
        self.vbox.addWidget(self.bagqav1)
        bqv1_hbox = QtWidgets.QHBoxLayout()
        self.bagqav1.setLayout(bqv1_hbox)
    ...
    def _ui_execute_bqv1(self):
        ...
        button.setText("BAG QA v1")
        button.setToolTip('Perform checks on BAGs')
        # noinspection PyUnresolvedReferences
        button.clicked.connect(self.click_bag_qa_v1)
    ...
    def click_bag_qa_v1(self):
        """trigger the bag qa v1"""
        self._click_bag_qa(1)


    @classmethod
    def click_open_manual_v2(cls):
        logger.debug("open manual")
        Helper.explore_folder("https://www.hydroffice.org/manuals/qctools/user_manual_bag_qa.html")

    def _click_bag_qa(self, version):
        ...
        url_suffix = "survey_bag_qa_%d" % version
        ...
        for i, grid_file in enumerate(grid_list):
            ...
            if version == 1:
                success, report = self._bag_qa(grid_file=grid_file, version=version, idx=(i + 1), total=len(grid_list))
            else:  # this case should be never reached after the sanity checks
                raise RuntimeError("unknown Bag QA version: %s" % version)
            ...
            report_filename = 'BAG_QA_output_'+datetime_string+'.txt'
            output_folder = self.prj.gridqa_output_folder
        ...
        QtWidgets.QMessageBox.information(self, "Bag QA v%d" % version, msg, QtWidgets.QMessageBox.Ok)

    def _bag_qa(self, grid_file, version, idx, total):
        # MATT: This will need attention with addition of your code

        logger.debug('bag qa v%d ...' % version)

        self.prj.progress.start(title="BAG QA v.%d" % version,
        ...
        self.prj.progress.update(value=10,
                                 text="BAG QA v%d [%d/%d]" % (version, idx, total))
        ...
        output_xml = os.path.join(self.prj.gridqa_output_folder, grid_file+'_metadata')
        ...
        if w.has_uncertainty():
            ...
            if len(high_uncert_tracker)+len(zero_uncert_tracker)+len(negative_uncert_tracker)+len(nan_uncert_tracker)==0:
