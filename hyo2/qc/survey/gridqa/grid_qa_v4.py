...
class GridQAV4(BaseGridQA):
    ...
    def __init__(self, grids, force_tvu_qc, has_depth, has_product_uncertainty, has_density, has_tvu_qc, output_folder,
        ...
        super(GridQAV4, self).__init__(grids=grids)
        self.type = qa_algos["GRID_QA_v4"]
    ...
    def run(self):
        ...
        if self.has_density:
            ...
            # save the histogram as png
            density_png_file = "%s.QAv4.density.png" % os.path.splitext(self.grids.current_basename)[0]
        ...
        if (self.has_tvu_qc and not self.force_tvu_qc) or self.has_product_uncertainty:
            ...
            # save the histogram as png
            tvu_qc_png_file = "%s.QAv4.tvu_qc.png" % os.path.splitext(self.grids.current_basename)[0]
        ...
        if self.grids.is_vr():
            ...
            if self.objection_detection:
                ...
                # save the histogram as png
                pct_od_png_file = "%s.QAv4.pct_res.obj_det.png" % os.path.splitext(self.grids.current_basename)[0]
                ...
                GridQAV4.plot_hysto(layer_name="RES OD", bins=pct_od_bins, density=pct_od_density,
            ...
            if self.full_coverage:
                ...
                # save the histogram as png
                pct_cc_png_file = "%s.QAv4.pct_res.full_cov.png" % os.path.splitext(self.grids.current_basename)[0]
                ...
                GridQAV4.plot_hysto(layer_name="RES FC", bins=pct_cc_bins, density=pct_cc_density,
        ...
        if self.grids.is_vr():
            ...
            if self.full_coverage:
                if len(self.pct_cc_values) > 0:
                    ...
                    for pct_od in self.pct_cc_values:
                        # logger.debug("- value: %s" % pct_od)
                        pct_cc_key = int(round(pct_od * self.pct_cc_mul))
                        self.pct_cc_dict[pct_cc_key] += 1

    def _init_infos(self):

        self.bathy_info = GridInfoV4()
        ...
        self.tvu_qc_info = GridInfoV4()
        self.tvu_qc_info.title = "Uncertainty Standards"
    ...
    def _create_arrays(self):
        ...
        if self.has_density:
            ...
            if density_type == GRIDS_UINT32:
                ...
                if len(self.density_values) == 0:
                    raise RuntimeError("No density values")
            else:
        ...
        if (self.has_tvu_qc and not self.force_tvu_qc) or self.has_product_uncertainty:
            ...
            if self.has_tvu_qc and not self.force_tvu_qc:
                ...
                # logger.debug("tvu qc layer: %s [idx: %s]"
                #              % (self.grids.grid_data_type(tvu_qc_type), tvu_qc_idx))
                if tvu_qc_type == GRIDS_DOUBLE:
                    self.tvu_qc_values = \
                        tile.doubles[tvu_qc_idx][tile.doubles[tvu_qc_idx] != tile.doubles_nodata[tvu_qc_idx]]
                    if len(self.tvu_qc_values) == 0:
                        raise RuntimeError("No TVU QC values")
    ...
    def plot_hysto(cls, layer_name, bins, density, bin_width, grid_info, png_path):
        ...
        if grid_info.pct_of_passed_nodes is None:
            ...
            tmp_str = "%s%% pass (%s of all nodes)" % (pct_pass, '{:,}'.format(grid_info.nr_of_passed_nodes))
        ...
        for i in ((grid_info.p2_5 <= bins) & (bins <= grid_info.p97_5)).nonzero()[0]:
            shaded_colors[i] = GridQAV4.RGB_PuBu2

        # noinspection PyUnresolvedReferences
        for i in ((grid_info.q1 <= bins) & (bins <= grid_info.q3)).nonzero()[0]:
            shaded_colors[i] = GridQAV4.RGB_PuBu3
        ...
        fig.text(.5, .935, grid_info.title, fontsize=18, ha='center')
    ...
    def _finish_plot_depth_vs_density(self):
        ...
        png_file = "%s.QAv4.depth_vs_density.png" % os.path.splitext(self.grids.current_basename)[0]
    ...
    def _finish_plot_depth_vs_tvu_qc(self):
        ...
        png_file = "%s.QAv4.depth_vs_tvu_qc.png" % os.path.splitext(self.grids.current_basename)[0]
        png_path = os.path.join(self.output_folder, png_file)
        png_path = Helper.truncate_too_long(png_path, left_truncation=True)
