...
from hyo2.qc.survey.anomaly.anomaly_detector_v1_checks import \
    check_laplacian_operator, check_gaussian_curvature, \
    check_adjacent_cells, check_small_groups, check_noisy_edges
from hyo2.qc.survey.anomaly.ad_base import AnomalyDetectorBase, anomaly_algos
...
class AnomalyDetectorV1(AnomalyDetectorBase):

    # default_filter_distance = 1.0
    # default_filter_delta_z = 0.01

    def __init__(self, grid, height=None,
    ...
    #     self.cur_th_height = None
    #     self.cur_th_curv = None
    ...
    #
    #     if self.flier_height is not None:
    #         logger.debug("using user-selected height: %s" % self.flier_height)
    #         self.cur_th_height = np.empty_like(self.dtm_mask, dtype=np.float32)
    #         self.cur_th_height[self.dtm_mask.mask] = np.nan
    #         self.cur_th_height[~self.dtm_mask.mask] = self.flier_height
    #     else:
    #         self.cur_th_height = self.ths.th_height
    #     self.cur_th_curv = self.ths.th_gauss_curv
    #
    ...
    #
    #     th = -4. * self.cur_th_height
    #
    #     lap = ndimage.filters.laplace(self.bathy_values)
    #
    #     check_laplacian_operator(lap, self.flag_grid, th)
    ...
    #
    #     # check_gaussian_curvature(self.gauss_curv, self.flag_grid, self.cur_th_curv)
    ...
    #     check_adjacent_cells(self.bathy_values, self.flag_grid, self.cur_th_height, 0.75, 0.8)
    ...
    #     check_noisy_edges(self.bathy_values, self.flag_grid)
    #
    ...
    #     else:
    #         th = self.cur_th_height / 2.0
    #
    #     # make a binary image
    #     grid_bin = np.isfinite(self.bathy_values)
    #
    #     check_small_groups(grid_bin, self.bathy_values, self.flag_grid, th, area_limit, self.check_slivers,
    #                              self.check_isolated)
    ...
    #         if len(self.bathy_values) == 0:
    #             raise RuntimeError("No bathy values")
    #         self.bathy_values = self.bathy_values.astype(np.float64)
    #
    #     else:
