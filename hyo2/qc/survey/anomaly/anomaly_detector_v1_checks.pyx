...
#@cython.profile(True)
cpdef check_laplacian_operator_double(double[:, :] lap, int[:, :] flag_grid, double[:, :] th):
    ...
    for r in range(lap_rows):
        for c in range(lap_cols):
            if npy_isnan(th[r,c]):
                continue
            if (lap[r,c] < th[r,c]) or (lap[r,c] > -th[r,c]):
...
@cython.nonecheck(False)
#@cython.profile(True)
cpdef check_gaussian_curvature_double(double[:, :] gauss_curv, int[:, :] flag_grid, double[:, :] th):
    ...
    for r in range(lap_rows):
        for c in range(lap_cols):
            if npy_isnan(th[r,c]):
                continue
            if gauss_curv[r,c] > th[r,c]:
                flag_grid[r, c] = 2 # check #2
                logging.info("#2 > @(%d,%d) -> %f > %f" % (int(r), int(c), gauss_curv[r,c], th[r,c]))
...
@cython.nonecheck(False)
#@cython.profile(True)
cpdef check_adjacent_cells_double(double[:, :] bathy, int[:, :] flag_grid, double[:, :] th, float pct1, float pct2):
            ...
            for c in range(4, cols - 4):  # skip bbox boundaries
                ...
                for ni in range(nbs_sz):
                    ...
                    if (nl != 0) and (nl != i) and (sizes[nl - 1] > area_limit):
                        ...
                        if (abs(bathy[r, c] - bathy[nb_rs[ni], nb_cs[ni]]) > th[r,c]) \
                                and check_slivers:
