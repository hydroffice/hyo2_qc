...
class Checks:
    ...
    def all_the_features(self):
        self.report += "Checks for all the features [SECTION]"
    ...
    def _all_features_redundancy_and_geometry(self) -> None:
        ...
        for ft in self.all_fts:
            ...
            if i > 0:  # we have a redundancy
                if ft.acronym in ["LIGHTS", ]:
                    # add to the flagged feature list
                    self.flags.append(ft.centroid.x, ft.centroid.y, "possible redundant %s" % ft.acronym)
                    # add to the flagged report
        ...
        logger.debug("nr. of features after redundancy/geometry checks: %d" % len(tmp_features))
        self.all_fts = tmp_features
    ...
    def new_or_updated_features(self):
        ...
        self.flags.sorind = self._check_features_for_attribute(
        ...
        if self.sorind is None:
            self.flags.sorind_invalid = self._check_features_for_valid_sorind(
                objects=self.new_updated_fts,
                check_space=False)
        else:
            self.flags.sorind_invalid = self._check_features_for_match_sorind(
                objects=self.new_updated_fts)
        ...
        self.flags.sordat = self._check_features_for_attribute(
            objects=self.new_updated_fts,
            attribute='SORDAT')
        ...
        if self.sordat is None:
            self.flags.sordat_invalid = self._check_features_for_valid_sordat(self.new_updated_fts)
        else:
            self.flags.sordat_invalid = self._check_features_for_match_sordat(self.new_updated_fts)
        ...
        if self.use_mhw:
            ...
            self.flags.new_valsous_watlev = self._check_features_for_valid_watlev(
                objects=new_valsous)
        ...
        self.flags.new_elevat = self._check_features_for_valid_elevat(
            objects=new_elevats)
        ...
        if self.version in ["2020", "2021"]:
            ...
            self.flags.new_valsous_quasou = self._check_features_for_valid_quasou(new_valsous)
    ...
    def _check_features_for_valid_watlev(self, objects: List['S57Record10']) -> List[list]:
        ...
        for obj in objects:
            ...
            if self.version in ["2019", "2020", "2021"]:
                ...
                elif self.survey_area == self.survey_areas["Pacific Coast"]:
                ...
                elif self.survey_area == self.survey_areas["Atlantic Coast"]:
    ...
    def assigned_features(self):
        ...
        self.flags.description = self._flag_features_with_attribute_value(
        ...
        self.flags.remarks = self._check_features_for_attribute(
            objects=self.assigned_fts,
            attribute='remrks')
    ...
    def new_or_deleted_features(self):
        ...
        self.flags.remarks_features = self._check_features_for_attribute(
        ...
        self.flags.recommend_features = self._check_features_for_attribute(
            objects=self.new_deleted_fts,
            attribute='recomd')
    ...
    def images(self):
        ...
        self.flags.images_hssd = self._check_features_for_images(objects=self.all_fts)
        ...
        if self.version in ["2019"]:
            ...
            if (self.profile == 1 and self.use_htd) or (self.profile == 0):
                ...
                self.flags.images_sbdare_points = self._check_sbdare_images_per_htd(objects=sbdare_points)

                self.report += "Invalid IMAGE name per HTD 2018-5 [CHECK]"
                self.flags.images_non_sbdare = self._check_nonsbdare_images_per_htd(objects=non_sbdare_features)

                # Isolate new or update line and area seabed areas
                self.report += "SBDARE IMAGE name per HTD 2018-5 [CHECK]"
                self.flags.images_sbdare_lines_areas = self._check_nonsbdare_images_per_htd(objects=sbdare_lines_areas)

        elif self.version in ["2020", "2021"]:

            self.report += "Invalid bottom sample IMAGE name per HSSD [CHECK]"
            self.flags.images_sbdare_points = self._check_sbdare_images_per_htd(sbdare_points)

            self.report += "Invalid feature IMAGE name per HSSD [CHECK]"
            self.flags.images_features = self._check_nonsbdare_images_per_htd(
    ...
    def soundings(self):
        ...
        self.flags.soundings_tecsou = self._flag_features_with_attribute_value(
        ...
        self.flags.soundings_quasou = self._flag_features_with_attribute_value(
    ...
    def dtons(self):
        ...
        self.flags.dtons = self._check_features_for_attribute(
            objects=dtons,
            attribute='images')
    ...
    def wrecks(self):
        ...
        self.flags.wrecks_images = self._check_features_for_attribute(
            objects=wrecks,
            attribute='images')
        ...
        self.flags.wrecks_catwrk = self._flag_features_with_attribute_value(
            objects=wrecks,
            attribute='CATWRK',
            values_to_flag=['', ],
            check_attrib_existence=True)
        ...
        if self.version in ["2020", "2021"]:
            ...
            self.flags.wrecks_watlev = self._flag_features_with_attribute_value(wrecks_valsou,
                                                                                attribute='WATLEV',
                                                                                values_to_flag=['', ],
                                                                                check_attrib_existence=True)
            ...
            self.flags.wrecks_unknown_watlev = self._flag_features_with_attribute_value(wrecks_undefined_valsou,
                                                                                        attribute='WATLEV',
        ...
        self.flags.wrecks_valsou = self._check_features_for_attribute(
            objects=wrecks,
            attribute='VALSOU')
        ...
        self.flags.wrecks_tecsou = self._flag_features_with_attribute_value(
            objects=wrecks,
            attribute='TECSOU',
            values_to_flag=['', ],
            check_attrib_existence=True)
        ...
        if self.version in ["2020", "2021"]:
            ...
            self.flags.wrecks_unknown_tecsou = self._flag_features_with_attribute_value(wrecks_undefined_valsou,
                                                                                        attribute='TECSOU',
                                                                                        values_to_flag=["1", "2", "3",
                                                                                                        "4", "5", "6",
                                                                                                        "7", "8", "9",
    ...
    def rocks(self):
        ...
        if self.version in ["2019", ]:
            ...
            self.report += "New or Updated UWTROC with empty/missing mandatory attribute WATLEV [CHECK]"
            self.flags.uwtroc_watlev = self._flag_features_with_attribute_value(rocks_valsou, attribute='WATLEV',
                                                                                values_to_flag=['', ],
                                                                                check_attrib_existence=True)

            self.report += "New or Updated UWTROC with empty VALSOU shall have WATLEV of 'unknown' [CHECK]"
            self.flags.uwtroc_unknown_watlev = self._flag_features_with_attribute_value(rocks_undefined_valsou,
                                                                                        attribute='WATLEV',
                                                                                        values_to_flag=["1", "2", "3",
                                                                                                        "4", "5", "6",
                                                                                                        "7", ],
        ...
        if self.version in ["2019", ]:
            ...
            self.report += "New or Updated UWTROC with empty/missing mandatory attribute QUASOU [CHECK]"
            self.flags.uwtroc_quasou = self._flag_features_with_attribute_value(rocks_valsou, attribute='QUASOU',
                                                                                values_to_flag=['', ],
                                                                                check_attrib_existence=True)
            ...
            self.flags.uwtroc_unknown_quasou = self._flag_features_with_attribute_value(rocks_undefined_valsou,
                                                                                        attribute='QUASOU',
                                                                                        values_to_flag=["1", "3", "4",
                                                                                                        "5",
                                                                                                        "6", "7", "8",
        ...
        if self.version in ["2019", ]:
            ...
            # @ Ensure new or updated rocks have tecsou
            self.report += "New or Updated UWTROC with empty/missing mandatory attribute TECSOU [CHECK]"
            self.flags.uwtroc_tecsou = self._flag_features_with_attribute_value(rocks_valsou,
                                                                                attribute='TECSOU',
                                                                                values_to_flag=['', ],
                                                                                check_attrib_existence=True)
            # Ensure rocks with unknown valsou have tecsou "unknown"
            self.report += "New or Updated UWTROC with empty VALSOU shall have TECSOU of 'unknown' [CHECK]"
            self.flags.uwtroc_unknown_tecsou = self._flag_features_with_attribute_value(rocks_undefined_valsou,
                                                                                        attribute='TECSOU',
                                                                                        values_to_flag=["1", "2", "3",
                                                                                                        "4", "5", "6",
                                                                                                        "7", "8", "9",
    ...
    def obstructions(self):
        ...
        self.flags.obstrn_images = self._check_features_for_attribute(
        ...
        self.flags.obstrn_points_valsou = self._check_features_for_attribute(
        ...
        self.flags.obstrn_points_watlev = self._flag_features_with_attribute_value(
            objects=obstrn_points,
            attribute='WATLEV',
            values_to_flag=['', ],
            check_attrib_existence=True)
        ...
        self.flags.obstrn_lines_areas_watlev = self._flag_features_with_attribute_value(
        ...
        self.flags.obstrn_watlev_known = self._flag_features_with_attribute_value(
        ...
        if self.version in ["2019", ]:
            ...
            self.flags.obstrn_tecsou = self._flag_features_with_attribute_value(
                objects=obstrns,
                attribute='TECSOU',
                values_to_flag=['', ],
                check_attrib_existence=True)
            ...
            self.report += "New or Updated OBSTRN with empty/missing mandatory attribute QUASOU [CHECK]"
            self.flags.obstrn_quasou = self._flag_features_with_attribute_value(obstrn_valsou,
                                                                                attribute='QUASOU',
                                                                                values_to_flag=['', ],
                                                                                check_attrib_existence=True)
            ...
            self.flags.obstrn_unknown_quasou = self._flag_features_with_attribute_value(obstrn_undefined_valsou,
                                                                                        attribute='QUASOU',
            ...
            self.report += "New or Updated OBSTRN missing mandatory attribute TECSOU [CHECK]"
            self.flags.obstrn_tecsou = self._flag_features_with_attribute_value(obstrn_valsou,
                                                                                attribute='TECSOU',
                                                                                values_to_flag=['', ],
                                                                                check_attrib_existence=True)
            ...
            self.flags.obstrn_unknown_tecsou = self._flag_features_with_attribute_value(obstrn_undefined_valsou,
                                                                                        attribute='TECSOU',
                                                                                        values_to_flag=["1", "2", "3",
                                                                                                        "4", "5", "6",
                                                                                                        "7", "8", "9",
        ...
        if self.version in ["2019", ]:
            ...
            self.flags.obstrn_unknown_valsou = self._check_features_for_attribute(obstrns_no_foul_lines_areas,
                                                                                  'VALSOU',
                                                                                  possible=True)
    ...
    def platforms(self):
        ...
        self.flags.ofsplf = self._check_features_for_attribute(
            objects=ofsplf,
            attribute='images')
    ...
    def sbdares(self):
        ...
        self.flags.sbdare_natsur = self._flag_features_with_attribute_value(
        ...
        self.flags.sbdare_pt_natqua = self._check_sbdare_attribute_counts(
        ...
        self.flags.sbdare_pt_colour = self._check_sbdare_attribute_counts(
            sbdare_points=sbdare_points,
            limiting_attribute='NATSUR',
            dependent='COLOUR')
        ...
        self.flags.sbdare_pt_allowable_combo = self._allowable_sbdare(sbdare_points=sbdare_points)
        ...
        self.flags.sbdare_watlev = self._check_features_for_attribute(
            objects=sbdare_lines_areas,
            attribute='WATLEV',
            possible=True)
    ...
    def moorings(self):
        ...
        self.flags.morfac = self._flag_features_with_attribute_value(
            objects=morfac,
            attribute='CATMOR',
            values_to_flag=['', ],
            check_attrib_existence=True)
    ...
    def coastlines(self):
        ...
        self.flags.coalne = self._flag_features_with_attribute_value(
            objects=coalne,
            attribute='CATCOA',
            values_to_flag=['', ],
            check_attrib_existence=True)
        ...
        self.flags.slcons = self._flag_features_with_attribute_value(
            objects=slcons,
            attribute='CATSLC',
            values_to_flag=['', ],
            check_attrib_existence=True)
    ...
    def lands(self):
        ...
        self.flags.lndelv = self._check_features_for_attribute(
            objects=lndelv,
            attribute='ELEVAT')
    ...
    def coverages(self):
        ...
        self.flags.m_covr_catcov = self._flag_features_with_attribute_value(
            objects=mcovr,
            attribute='CATCOV',
            values_to_flag=['', ],
            check_attrib_existence=True)
        ...
        self.flags.m_covr_inform = self._check_features_for_attribute(
            objects=mcovr,
            attribute='INFORM')

        # Ensure M_COVR has ninfom
        self.report += "M_COVR missing mandatory attribute NINFOM [CHECK]"
        self.flags.m_covr_ninfom = self._check_features_for_attribute(
            objects=mcovr,
            attribute='NINFOM')
    ...
    def office_only(self):
        ...
        self.flags.without_onotes = self._check_features_for_attribute(
            objects=self.all_fts,
            attribute='onotes')

        # For the office profile, check for empty hsdrec
        self.report += "Features with empty/unknown attribute hsdrec [CHECK]"
        self.flags.hsdrec_empty = self._flag_features_with_attribute_value(
            objects=self.all_fts,
            attribute='hsdrec',
            values_to_flag=['', ],
            check_attrib_existence=True)
        ...
        self.flags.prohibited_kwds = self._check_for_missing_keywords(
            objects=prohibited,
            attr_acronym='onotes',
        ...
        self.flags.fish_haven_kwds = self._check_for_missing_keywords(
            objects=fish_haven,
            attr_acronym='onotes',
            keywords=['Prohibited feature', ])
        ...
        self.flags.mooring_buoy_kwds = self._check_for_missing_keywords(
        ...
        self.flags.m_qual_catzoc = self._flag_features_with_attribute_value(
            objects=mqual,
            attribute='CATZOC',
            values_to_flag=['', ],
            check_attrib_existence=True)
        ...
        self.flags.mcd_description = self._check_features_for_attribute(
            objects=self.all_fts,
            attribute='descrp')

        # Ensure all features have remrks (per MCD)
        self.report += "Features missing mandatory attribute remarks [CHECK]"
        self.flags.mcd_remarks = self._check_features_for_attribute(
            objects=self.all_fts,
            attribute='remrks')
        ...
        self.flags.character_limit = self._check_character_limit(
            objects=self.all_fts,
    ...
    def finalize_summary(self):
        ...
        all_fts_flagged = len(self.flags.redundancy)
        self.report += 'Section %s - Checks for all features: %s' % (chr(count), all_fts_flagged)
        count += 1

        # ### NEW OR UPDATED FEATURES ###

        nu_fts_flagged = \
            len(self.flags.sorind) + \
            len(self.flags.sorind_invalid) + \
            len(self.flags.sordat) + \
            len(self.flags.sordat_invalid) + \
            len(self.flags.new_elevat)
        if self.use_mhw:
            nu_fts_flagged += len(self.flags.new_valsous_watlev)
        if self.version in ["2020", "2021"]:
            nu_fts_flagged += len(self.flags.new_valsous_quasou)

        self.report += 'Section %s - Checks for new or updated features: %s' % (chr(count), nu_fts_flagged)
        count += 1

        # ### ASSIGNED FEATURES ###

        ass_fts_flagged = \
            len(self.flags.description) + \
            len(self.flags.remarks)
        self.report += 'Section %s - Checks for assigned features: %s' % (chr(count), ass_fts_flagged)
        count += 1

        # ### NEW OR DELETED FEATURES ###

        nd_fts_flagged = \
            len(self.flags.remarks_features) + \
            len(self.flags.recommend_features)
        self.report += 'Section %s - Checks for new or deleted features: %s' % (chr(count), nd_fts_flagged)
        count += 1
        ...
        if self.version in ["2019"]:
            if (self.profile == 1 and self.use_htd) or (self.profile == 0):
                img_flagged += \
                    ...
                    len(self.flags.images_non_sbdare) + \
                    len(self.flags.images_non_sbdare)
        elif self.version in ["2020", "2021"]:
            img_flagged += \
                len(self.flags.images_sbdare_points) + \
                len(self.flags.images_features)
        self.report += 'Section %s - Checks for images: %s' % (chr(count), img_flagged)
        count += 1

        # SOUNDINGS

        snd_flagged = \
            len(self.flags.soundings_tecsou) + len(self.flags.soundings_quasou)
        self.report += 'Section %s - Checks for soundings: %s' % (chr(count), snd_flagged)
        count += 1

        # DTONS

        dtn_flagged = \
            len(self.flags.dtons)
        self.report += 'Section %s - Checks for DTONs: %s' % (chr(count), dtn_flagged)
        count += 1

        # WRECKS

        wrk_flagged = \
            len(self.flags.wrecks_images) + \
            len(self.flags.wrecks_catwrk) + \
            len(self.flags.wrecks_watlev) + \
            len(self.flags.wrecks_valsou) + \
            len(self.flags.wrecks_tecsou) + \
            len(self.flags.wrecks_quasou)
        if self.version in ["2020", "2021"]:
            wrk_flagged += \
                len(self.flags.wrecks_unknown_watlev) + \
                len(self.flags.wrecks_unknown_tecsou) + len(self.flags.wrecks_unknown_quasou)
        self.report += 'Section %s - Checks for wrecks: %s' % (chr(count), wrk_flagged)
        count += 1

        # ROCKS

        rck_flagged = \
            len(self.flags.uwtroc_valsou) + \
            len(self.flags.uwtroc_watlev) + \
            len(self.flags.uwtroc_quasou) + \
            len(self.flags.uwtroc_tecsou)
        if self.version in ["2020", "2021"]:
            rck_flagged += \
                len(self.flags.uwtroc_unknown_watlev) + \
                len(self.flags.uwtroc_unknown_quasou) + \
                len(self.flags.uwtroc_unknown_tecsou)
        self.report += 'Section %s - Checks for underwater rocks: %s' % (chr(count), rck_flagged)
        count += 1

        # OBSTRUCTIONS

        obs_flagged = \
            len(self.flags.obstrn_images) + \
            len(self.flags.obstrn_points_valsou) + \
            len(self.flags.obstrn_points_watlev) + \
            len(self.flags.obstrn_lines_areas_watlev) + \
            len(self.flags.obstrn_watlev_known) + \
            len(self.flags.obstrn_quasou) + \
            len(self.flags.obstrn_tecsou) + \
            len(self.flags.obstrn_foul_valsou)
        if self.version in ["2020", "2021"]:
            obs_flagged += \
                len(self.flags.obstrn_unknown_quasou) + \
                len(self.flags.obstrn_unknown_tecsou) + \
                len(self.flags.obstrn_unknown_valsou)
        self.report += 'Section %s - Checks for obstructions: %s' % (chr(count), obs_flagged)
        count += 1

        # OFFSHORE PLATFORMS

        plt_flagged = \
            len(self.flags.ofsplf)
        self.report += 'Section %s - Checks for offshore platforms: %s' % (chr(count), plt_flagged)
        count += 1

        # SEABED AREAS

        sbd_flagged = \
            len(self.flags.sbdare_natsur) + \
            len(self.flags.sbdare_pt_natqua) + \
            len(self.flags.sbdare_pt_colour) + \
            len(self.flags.sbdare_pt_allowable_combo) + \
            len(self.flags.sbdare_watlev)
        self.report += 'Section %s - Checks for offshore platforms: %s' % (chr(count), sbd_flagged)
        count += 1

        # MOORINGS

        mor_flagged = \
            len(self.flags.morfac)
        self.report += 'Section %s - Checks for mooring facilities: %s' % (chr(count), mor_flagged)
        count += 1

        # COASTLINES

        cst_flagged = \
            len(self.flags.coalne) + \
            len(self.flags.slcons)
        self.report += 'Section %s - Checks for mooring facilities: %s' % (chr(count), cst_flagged)
        count += 1

        # LANDS

        lnd_flagged = \
            len(self.flags.lndelv)
        self.report += 'Section %s - Checks for land elevations: %s' % (chr(count), lnd_flagged)
        count += 1

        # META COVERAGES

        cvr_flagged = \
            len(self.flags.m_covr_catcov) + \
            len(self.flags.m_covr_inform) + \
            len(self.flags.m_covr_ninfom)
        self.report += 'Section %s - Checks for meta coverages: %s' % (chr(count), cvr_flagged)
        count += 1
        ...
        if self.profile == 0:  # office profile
            ...
            ofc_flagged = \
                len(self.flags.without_onotes) + \
                len(self.flags.hsdrec_empty) + \
                len(self.flags.prohibited_kwds) + \
                len(self.flags.fish_haven_kwds) + \
                len(self.flags.mooring_buoy_kwds) + \
                len(self.flags.m_qual_catzoc) + \
                len(self.flags.m_qual_sursta) + \
                len(self.flags.m_qual_surend) + \
                len(self.flags.m_qual_tecsou) + \
                len(self.flags.mcd_description) + \
                len(self.flags.mcd_remarks) + \
                len(self.flags.character_limit)
            self.report += 'Section %s - Checks ONLY for office: %s' % (chr(count), ofc_flagged)
            count += 1

    ########

    # @classmethod
    # def flag_features_by_type(cls, objects: List['S57Record10'],
    #                           flags: 'Flags', report: 'Report',
    #                           types_to_flag: List[str]) -> List[list]:
    #     """Flag the passed features if they have the passed values for the passed attribute"""
    #     flagged = list()
    #
    #     for obj in objects:
    ...
    #
    #         # add to the flagged feature list
    #         flags.append(obj.centroid.x, obj.centroid.y, "prohibited type %s" % obj.acronym)
    #         # add to the flagged report
    #         report += 'found prohibited type %s at (%s, %s)' % (obj.acronym, obj.centroid.x, obj.centroid.y)
    #         flagged.append([obj.acronym, obj.centroid.x, obj.centroid.y])
    #
    #     if len(flagged) == 0:
    #         report += "OK"
