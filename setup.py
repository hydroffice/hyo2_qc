...
setup(
    ...
    ext_modules=cythonize([
        ...
        Extension("hyo2.qc.survey.gridqa.grid_qa_calc_v4",
                  sources=["hyo2/qc/survey/gridqa/grid_qa_calc_v4.pyx"],
                  include_dirs=[np.get_include()],
                  language='c++',
                  ),
        Extension("hyo2.qc.survey.anomaly.anomaly_detector_v1_checks",
                  sources=["hyo2/qc/survey/anomaly/anomaly_detector_v1_checks.pyx"],
                  include_dirs=[np.get_include()],
                  language='c++',
                  # extra_compile_args=["-Zi", "/Od"],
                  # extra_link_args=["-debug"],
                  ),
        Extension("hyo2.qc.survey.anomaly.anomaly_detector_v1_proxies",
                  sources=["hyo2/qc/survey/anomaly/anomaly_detector_v1_proxies.pyx"],
                  include_dirs=[np.get_include()],
                  language='c++',
                  # extra_compile_args=["-Zi", "/Od"],
                  # extra_link_args=["-debug"],
...
,
    ...
    author="Giuseppe Masetti(UNH,CCOM); Tyanne Faulkes(NOAA,OCS); Brian Calder(UNH,CCOM)",
    author_email="gmasetti@ccom.unh.edu; tyanne.faulkes@noaa.gov; brc@ccom.unh.edu",
)
