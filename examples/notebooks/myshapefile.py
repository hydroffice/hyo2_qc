...
class Reader(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        print("encoding: %s, error: %s" % (self.encoding, self.encodingErrors))
    ...
    def __shpHeader(self):
        ...
        shp = self.shp
        # file code
        fileCode = unpack(">i", shp.read(4))[0]
        print("fileCode: %s" % hex(fileCode))
        # unused 4 x int32
        unuseds = unpack(">5i", shp.read(20))
        print("unuseds: %s" % (unuseds,))
        # File length (16-bit word * 2 = bytes)
        shp.seek(24)
        self.shpLength = unpack(">i", shp.read(4))[0] * 2
        print("shp length: %d" % self.shpLength)
        # unused 4 x int32
        version = unpack("<i", shp.read(4))
        print("version: %s" % (version,))
        # Shape type
        shp.seek(32)
        self.shapeType= unpack("<i", shp.read(4))[0]
        print("shp type: %d" % self.shapeType)
        # The shapefile's bounding box (lower left, upper right)
        self.bbox = _Array('d', unpack("<4d", shp.read(32)))
        print("bbox: %s" % (self.bbox, ))
        # Elevation
        self.zbox = _Array('d', unpack("<2d", shp.read(16)))
        print("zbox: %s" % (self.zbox, ))
        # Measure
        self.mbox = []
    ...
    def shapes(self):
        ...
        print("nr. of shapes: %s" % len(shapes))
        return shapes
    ...
    def __dbfHeader(self):
        ...
        print("numRecords: %s" % self.numRecords)
        # read fields
